...
public class BranchEvolutionManager {
    ...
    public String getOrCreateEvolutionBranch() {
        try {
            // Get the existing branch or create a new one if none exists
            String branchName = getExistingBranchName().orElseGet(this::createNewBranch);
            checkoutBranch(branchName); // Check out the determined branch
            return branchName;
        } catch (IOException e) {
    ...
    private Optional<String> getExistingBranchName() throws IOException {
        ...
        if (checkpoint.exists()) {
            // Read the branch name from the checkpoint file
    ...
    private void runCommand(String[] command) throws IOException {
        ...
        pb.inheritIO(); // Redirect I/O streams for easier debugging
        try {
